# How to run CusADi quickly 

## Installation
```bash
uv venv --python 3.11
source .venv/bin/activate
uv pip install casadi 
uv pip install -e .
```

## Code generation 
```bash
uv run run_codegen.py --fn g1_29dof_ang_momentum_func
```
...
# load cusadi func
num_envs = 4096
casadi_func_path = os.path.join(CASADI_FUNCTION_DIR, "g1_29dof_ang_momentum_func.casadi")
casadi_func = casadi.Function.load(casadi_func_path)
cusadi_func = CusadiFunction(casadi_func, num_instances=num_envs)

# evaluate function 
input_tensors = ...
cusadi_func.evaluate(input_tensors)
output_tensors = cusadi_func.getDenseOutput(0)
```
